...
def compute_data_metrics(batch, use_critic=True):
    ...
    sequence_answer_correct = batch.batch['answer_correct']
    sequence_search_key_score = batch.batch['search_key_score']
    sequence_step_scores = batch.batch['step_scores']
    ...
    metrics = {
        ...
        # answer correct
        'critic/answer_correct/mean':
            sum(sequence_answer_correct) / len(sequence_answer_correct),
        'critic/answer_correct/max':
            max(sequence_answer_correct),
        'critic/answer_correct/min':
            min(sequence_answer_correct),

        # search key score
        'critic/search_key_score/mean':
            sum(sequence_search_key_score) / len(sequence_search_key_score),
        'critic/search_key_score/max':
            max(sequence_search_key_score),
        'critic/search_key_score/min':
            min(sequence_search_key_score),

        # step scores
        'critic/step_scores/mean':
            sum(sequence_step_scores) / len(sequence_step_scores),
        'critic/step_scores/max':
            max(sequence_step_scores),
        'critic/step_scores/min':
            min(sequence_step_scores),
    ...
    def _validate(self):
        ...
        if not self.config.do_search:
            for test_data in self.val_dataloader:
                ...
                reward_tensor, answer_correct, _, _, _ = self.val_reward_fn(test_batch)
            ...
            for batch_dict in self.val_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    # evaluate using reward_function
                    # for certain reward function (e.g. sandbox), the generation can overlap with reward
                    reward_tensor, answer_correct, _, _ = self.val_reward_fn(test_batch)
        ...
        if self.config.trainer.save_predictions:
            ...
            prediction_save_dir = os.path.join(self.config.trainer.default_local_dir, "predictions") 
            os.makedirs(prediction_save_dir, exist_ok=True)
            
            # Save predictions to file
            # Save to JSON file
            output_file = os.path.join(prediction_save_dir, f'{timestamp}.json')
        ...
        for i, correct in enumerate(answer_correct_lst):
            data_source = data_sources[i]
            if data_source not in data_source_score:
                data_source_score[data_source] = []
            ...
            for batch_dict in self.train_dataloader:
                ...
                # with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        batch.batch['answer_correct'] = answer_correct
                        batch.batch['search_key_score'] = search_key_score
                        batch.batch['step_scores'] = step_scores
