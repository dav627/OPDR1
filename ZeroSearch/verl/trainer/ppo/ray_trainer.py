...
def compute_data_metrics(batch, trajectory_turns, use_critic=True):
    ...
    metrics = {
        ...
        **({
        ...
        # response length
        "response_length/turns": np.mean(trajectory_turns),
...
class RayPPOTrainer(object):
    ...
    def _validate(self, identifier):
        ...
        """
        import torch
        reward_tensor_lst_em = []
        reward_tensor_lst_f1 = []
        data_source_lst_em = []
        data_source_lst_f1 = []
        ...
        all_inference_data = []
        if not self.config.trainer.do_search:
            for test_data in tqdm(self.val_dataloader, desc="Testing"):
                ...
                test_gen_batch = test_batch.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids'], non_tensor_batch_keys=['question', 'golden_answers'])
                ...
                reward_tensor_em, reward_tensor_f1, inference_data = self.val_reward_fn(test_batch)
                all_inference_data.extend(inference_data)

                reward_tensor_lst_em.append(reward_tensor_em)
                reward_tensor_lst_f1.append(reward_tensor_f1)
                data_source_lst_em.append(
                    test_batch.non_tensor_batch.get('data_source', ['unknown'] * reward_tensor_em.shape[0]))
                data_source_lst_f1.append(
        ...
        data_sources_em = np.concatenate(data_source_lst_em, axis=0)
        data_sources_f1 = np.concatenate(data_source_lst_f1, axis=0)
        # evaluate test_score based on data source
        data_source_reward_em = {}
        for i in range(reward_tensor_em.shape[0]):
            data_source = data_sources_em[i]
            if data_source not in data_source_reward_em:
                data_source_reward_em[data_source] = []
            data_source_reward_em[data_source].append(reward_tensor_em[i].item())

        data_source_reward_f1 = {}
        for i in range(reward_tensor_f1.shape[0]):
            data_source = data_sources_f1[i]
            if data_source not in data_source_reward_f1:
                data_source_reward_f1[data_source] = []
            data_source_reward_f1[data_source].append(reward_tensor_f1[i].item())


        metric_dict = {}
        for data_source, rewards in data_source_reward_em.items():
            metric_dict[f'val/test_score_EM/{data_source}'] = np.mean(rewards)

        for data_source, rewards in data_source_reward_f1.items():
            metric_dict[f'val/test_score_F1/{data_source}'] = np.mean(rewards)
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get('val_before_train', True):
            val_metrics = self._validate(f'{self.config.trainer.default_local_dir}/record_init.json')
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in tqdm(self.train_dataloader, total=self.total_training_steps, desc="Training"):
                ...
                gen_batch = batch.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids'], non_tensor_batch_keys=['question', 'golden_answers'])
                ...
                with _timer('step', timing_raw):
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer('update_actor', timing_raw):
                            if self.config.trainer.do_search and self.config.actor_rollout_ref.actor.state_masking:
                    ...
                    if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and \
                        ...
                        with _timer('testing', timing_raw):
                            val_metrics: dict = self._validate(f'{self.config.trainer.default_local_dir}/record_{self.global_steps}.json')
                ...
                # collect metrics
                metrics.update(compute_data_metrics(batch=batch, trajectory_turns=trajectory_turns, use_critic=self.use_critic))
